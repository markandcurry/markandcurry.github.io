$(document).ready(function() {
    ...
    Carousel = function() {
		...
		$(carouselItems).each(function(index, value){
			...
			var liDot = $('<li/>');
			liDot.data('position', index); // Store the position of the dots
			liDot.data('title',value.title); // Pass the title to each dot
		...
		for (i = 0; i < $('.carousel-dots-nav-item').length; i++) {
			...
			$(dot).hover(function(e){
				$('#title').text($(this).data('title')); // title declared on line 27
			...
			$(dot).click(function(e){
				var position= $(this).data('position');  // Position declared on line 26
				$('#carousel').animate({
					left: -position * 100 + '%'
				}, 500);
			});
